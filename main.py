...
def tambah_data_administrasi():
    ...
    try:
        ...
        for i in range(total_input):
            ...
            query = """
            INSERT INTO administrasi (id_administrasi, pasien_id_pasien, dokter_id_dokter, perawat_id_perawat,\
                  ruangan_id_ruangan, pembayaran_id_pembayaran, obat_id_obat, admin_id_admin, tanggal_masuk, tanggal_keluar)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """
            cur.execute(query, (id_administrasi, id_pasien, id_dokter, id_perawat, id_ruangan,
                                 id_pembayaran, id_obat, id_admin, tanggal_masuk, tanggal_keluar))
...
def update_data_administrasi():
    ...
    try:
        ...
        table = PrettyTable()
        table.field_names = ["ID Administrasi", "ID Pasien", "ID Dokter", "ID Perawat", "ID Ruangan",
                              "ID Pembayaran", "ID Obat", "ID Admin", "Tanggal Masuk", "Tanggal Keluar"]
        for row in data1:
            table.add_row(row)
        ...
        if data2:
            table_single = PrettyTable()
            table_single.field_names = ["ID Administrasi", "ID Pasien", "ID Dokter", "ID Perawat", 
                                        "ID Ruangan", "ID Pembayaran", "ID Obat", "ID Admin", "Tanggal Masuk", "Tanggal Keluar"]
            table_single.add_row(data2)
            print(table_single)
            ...
            update_query = """
            UPDATE administrasi SET pasien_id_pasien = %s, dokter_id_dokter = %s, perawat_id_perawat = %s, ruangan_id_ruangan = %s,\
                  pembayaran_id_pembayaran = %s, obat_id_obat = %s, admin_id_admin = %s, tanggal_masuk = %s, tanggal_keluar = %s
...
def hapus_data_administrasi():
    ...
    try:
        ...
        if data2:
            table_single = PrettyTable()
            table_single.field_names = ["ID Administrasi", "ID Pasien", "ID Dokter", "ID Perawat", "ID Ruangan",
                                         "ID Pembayaran", "ID Obat", "ID Admin", "Tanggal Masuk", "Tanggal Keluar"]
            table_single.add_row(data2)
            print(table_single)
